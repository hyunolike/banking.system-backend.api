...
wait_until_up() {
  ...
  for _ in $(seq 1 "${HEALTH_WAIT_SECONDS}"); do
    ...
    # 인증이 필요한 경로라 401 이 떠도 서버가 떠 있다는 뜻이다.
    if curl -s -o /dev/null -m 2 "http://localhost:${port}/api/users/me"; then
